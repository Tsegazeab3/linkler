...
const GuidePreviewCard = ({ guide }) => {
  const { id, name, country, rating, review_count, price, serviceDescription, picture, user_id, is_following: initialIsFollowing } = guide;
  const { user } = useAuth();
      ...
      <CardContent className="p-5">
        <div className="flex justify-between items-start mb-1">
          ...
          <div className="flex items-center bg-brand-light px-2 py-1 rounded-lg">
            ...
            <span className="text-xs font-bold text-brand">{rating > 0 ? rating : 'New'}</span>
            {review_count > 0 && (
              <span className="text-[10px] text-ui-muted ml-1">({review_count})</span>
            )}
          </div>
        </div>
